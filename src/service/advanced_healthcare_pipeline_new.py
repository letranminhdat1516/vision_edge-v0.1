...
class AdvancedHealthcarePipeline:
    ...
    def process_frame(self, frame):
        ...
        # Lưu frame trước để tính motion  
            ...
            try:
                ...
                if seizure_result.get('temporal_ready'):
                    ...
                    if final_seizure_confidence > seizure_threshold:
                        ...
                        self.detection_history['seizure_confirmation_frames'] = max(0, self.detection_history['seizure_confirmation_frames'] - 1)
    ...
    def calculate_motion_level_person(self, person_detections):
        """Calculate motion level based on person detections như file mẫu"""
        if not person_detections or len(self.detection_history['motion_levels']) < 2:
            return 0.0
        
        # Use motion variance as motion level
    ...
    def enhance_detection_with_motion(self, base_confidence, motion_level, detection_type):
        """Enhanced detection with motion analysis như file mẫu"""
        if detection_type == 'fall':
            # Fall detection benefits from motion
            if motion_level > 0.6:
                enhancement = min(0.2, motion_level * 0.3)
                return min(1.0, base_confidence + enhancement)
            elif motion_level < 0.1:
                # Very low motion reduces fall confidence
                return base_confidence * 0.7
        elif detection_type == 'seizure':
            # Seizure detection needs sustained motion
            if motion_level > 0.7:
                enhancement = min(0.15, motion_level * 0.2)
                return min(1.0, base_confidence + enhancement)
            elif motion_level < 0.2:
                # Very low motion reduces seizure confidence more
