...
def create_statistics_state() -> Dict[str, Any]:
    ...
    return {
        ...
        'current_fps': 0.0,
        'last_fall_detection_time': 0,  # Thêm cooldown
        'fall_cooldown_seconds': 3  # 3 giây cooldown
    }
...
def update_fall_statistics(stats_state: Dict[str, Any], confidence: float) -> Dict[str, Any]:
    """
    Cập nhật thống kê khi detect fall (với cooldown)
    ...
    # Check cooldown
    time_since_last_fall = current_time - stats_state.get('last_fall_detection_time', 0)
    cooldown_seconds = stats_state.get('fall_cooldown_seconds', 3)
    
    if time_since_last_fall < cooldown_seconds:
        # Still in cooldown, don't count this detection
        return stats_state
    ...
    updated_stats['last_fall_detection_time'] = current_time
